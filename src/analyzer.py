...
for q_nr in [8, 19, 20, 21, 22, 23, 25, 30]:
    df = prep.replace_all_but(
        df, column=d.question_numbers[q_nr], exceptions=d.answers[q_nr]
    )
...
for q_nr in [31, 33]:
    ids1 = list(
        df[df[d.question_numbers[q_nr]] == "No (-> please skip next question)"].index
    )
...
df["birth year"] = (
    df["Completion time"] - pd.to_timedelta(df["age [y]"] * 365, unit="D")
).dt.year
...
for g in df_generations.index:
    id_ = df[
        (df["birth year"].values >= df_generations["from"].loc[g])
        & (df["birth year"].values <= df_generations["to"].loc[g])
    ].index
...
utils.relative_numbers(
    df[(df["generation"] == "Baby Boomers") | (df["generation"] == "X")],
    "Do you have responsibility for personnel?",
)
utils.relative_numbers(
    df[(df["generation"] == "Y") | (df["generation"] == "Z")],
    "Do you have responsibility for personnel?",
)
...
utils.relative_numbers(
    df[(df["generation"] == "Y") | (df["generation"] == "Z")], d.question_numbers[23]
)
...
pdf_path = os.path.abspath(
    os.path.join(script_dir, "..", "..", f"{MOD}_combined_plots.pdf")
)

with PdfPages(pdf_path) as pdf:
    # ---- Participant Data analyses

    pltr.submissions_age_histogram(df, df_generations, f"{MOD}_hist_n_submissions")
    fig = plt.gcf()
    pdf.savefig(fig)  # Saves the current figure into the PDF
    plt.close(fig)  # Close the figure to free memory

    # generations - gender stacked barchart
    pltr.submissions_barchart(df_generations, f"{MOD}_bar_n_submissions")
    fig = plt.gcf()
    pdf.savefig(fig)  # Saves the current figure into the PDF
    plt.close(fig)  # Close the figure to free memory

    # professions bar chart plot
    pltr.bar_chart_generic(
        df, column="profession", x_sort="ascending", filename=f"{MOD}_bar_professions"
    )
    fig = plt.gcf()
    ...
    pltr.bar_chart_generic(
        df,
        column="relevant work experience [y]",
    ...
    pltr.bar_chart_generic(
        df, column="position", x_sort="ascending", filename=f"{MOD}_bar_position"
    )
    fig = plt.gcf()
    ...
    for q_nr in [11, 29, 38]:
        pltr.generation_rating_plot(
            df,
            d.rating_columns[q_nr],
            d.answers[q_nr],
            d.question_numbers[q_nr],
    ...
    for q_nr in [12, 26, 28, 41, 45]:
        pltr.generation_violin_generic(
            df, d.question_numbers[q_nr], f"{MOD}_{q_nr}_violin"
        )
        fig = plt.gcf()
        pdf.attach_note(f"question number {q_nr}")
        pdf.savefig(fig)  # Saves the current figure into the PDF
    ...
    for q_nr in [
        ...
        pltr.generation_perc_bar_generic(
            df,
            d.question_numbers[q_nr],
            d.answers[q_nr],
    ...
    for q_nr in [14, 24, 27, 32, 34]:
        ...
        pltr.generation_multichoice_hbars(
            df,
            d.question_numbers[q_nr],
            q_nr,
            d.answers[q_nr],
